...
public class Ex4 implements Ex4_GUI{
	...
	private static void drawShape(GUI_Shapeable g) {
		...
		if(gs instanceof Rect2D) {
			...
			Point2D p2 = r.getP2();
			if(isFill) {
				StdDraw_Ex4.filledRectangle((p1.x()+p2.x())/2, (p1.y()+p2.y())/2,r.getWidth()/2, r.getHeight()/2);
			}
			else { 
				StdDraw_Ex4.rectangle((p1.x()+p2.x())/2, (p1.y()+p2.y())/2,r.getWidth()/2, r.getHeight()/2);
			}
		}
		...
		public void mouseMoved(MouseEvent e) {
			if(_p1!=null) {
				...
				if(_mode.equals("Rect")) {
					...
					gs = new Rect2D(_p1,p2);
				}
				if(_mode.equals("Segment")) {
					Point2D p2 = new Point2D(x1,y1);
					gs = new Segment2D(_p1,p2);
				}
